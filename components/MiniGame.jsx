...
const MiniGame = () => {
  ...
  return (
    <main className="bg-slate-800 min-h-[100vh] text-white flex flex-col items-center justify-center gap-10">
      ...
      <section>
        <div
          className="result bg-green-800 p-3 px-12 rounded-md text-3xl hidden"
          ref={correctAnswerBox}
        >
          {/* You're right! {previousNo1} &times; {previousNo2} really is {previousNo1 * previousNo2} */}
          Correct!
        </div>
        <div
          className="result bg-red-800 p-3 px-12 rounded-md text-3xl hidden"
          ref={wrongAnswerBox}
        >
          Wrong!
        </div>
      ...
      <div
        onClick={checkAnswer}
        className="check select-none border py-4 px-24 text-2xl cursor-pointer text-blue-500 font-bold rounded-tl-full rounded-br-full hover:rounded-tr-full hover:rounded-bl-full hover:rounded-tl-none hover:rounded-br-none transition-all duration-300"
      >
        Check Your Answer
      </div>
    </main>
